...
export async function createApp({
  ...
  console.time("git");
  try {
    // Debug: Check environment and template info
    console.log("Template ID:", templateId);
    console.log("Template URL:", templates[templateId].repo);
    console.log("Freestyle API Key exists:", !!process.env.FREESTYLE_API_KEY);
    console.log("User Freestyle Identity:", user.freestyleIdentity);
    ...
    if (error instanceof Error) {
      ...
      throw new Error(`Failed to create app: ${JSON.stringify(error)}`);
    }
  }
}
